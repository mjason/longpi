// Example longpi extension (TypeScript): a `web_search` tool backed by Tavily.
//
...
// shell `export` needed).
//
// Extensions run in an embedded QuickJS host: modern TypeScript (type
// annotations are stripped automatically before running) with `fetch`,
...
export default function (longpi: any) {
  longpi.registerTool({
    ...
    async execute(args: { query: string; max_results?: number }, _ctx: { cwd: string }) {
